...
<body class="arabic">
    ...
    <?php
    $query = "SELECT byIP FROM is_blocked WHERE status = 1 AND byIP = '$ip'";
    // Execute the query asynchronously
    // Execute the query asynchronously
    $query = "SELECT byIP FROM is_blocked WHERE status = 1 AND byIP = '$ip'";

    // Execute the query asynchronously
    $checkipResult = mysqli_query($conn, $query);

    if ($checkipResult !== false && $checkipResult !== true) {
        // Query executed successfully, fetch the result
        $checkip = mysqli_reap_async_query($checkipResult);
        // Process $checkip as needed
    } elseif ($checkipResult === true) {
        // This branch handles successful non-select queries (INSERT, UPDATE, DELETE, etc.)
        // You can handle this case differently if needed
        echo "Query executed successfully but no result to fetch.";
    } else {
    ...
    // $checkip = mysqli_reap_async_query(mysqli_query($conn, $query));
