...
if ($client === true) {
    if (empty($leadName) || empty($leadContact)) {
        ...
        if ($dupf['leadName'] == $leadName && $dupf['leadContact'] == $leadContact && $dupf['project'] == $project && $dupf['language'] == $language) {
            ...
            $query = mysqli_query($con, "INSERT INTO client_campaign_leads (client_id, leadName, notes, leadContact, leadEmail, enquiryType, leadFor, leadType, developer, project, projectName, leadStatus, leadSource, feedback, language, addedBy, filename, ip, device, otp, country) VALUES ('$client_id', '$leadName', '$callTime', '$leadContact', '$leadEmail', '$enquiryType','$leadFor', '$leadType', '$developer', '$project', '$project', '$leadStatus', '$leadSource', '$feedback', '$language', '$addedBy', '$filename', '$ip', '$device', 'Not Verified', '$country')");
            if ($query) {
                if ($language == "English") {
